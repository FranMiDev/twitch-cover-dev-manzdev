(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))d(a);new MutationObserver(a=>{for(const e of a)if(e.type==="childList")for(const o of e.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&d(o)}).observe(document,{childList:!0,subtree:!0});function c(a){const e={};return a.integrity&&(e.integrity=a.integrity),a.referrerPolicy&&(e.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?e.credentials="include":a.crossOrigin==="anonymous"?e.credentials="omit":e.credentials="same-origin",e}function d(a){if(a.ep)return;a.ep=!0;const e=c(a);fetch(a.href,e)}})();class l extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}static get styles(){return`
      :host {
    ...
    `}connectedCallback(){this.render()}render(){this.shadowRoot.innerHTML=`
      ...
      `}}customElements.define("cover-html",l);class r extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}static get styles(){return`
      :host {
        --width: var(--cover-size);
        --height: var(--cover-size);
    ...
    `}}customElements.define("cover-css",r);class i extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}static get styles(){return`
      :host {
        --width: var(--cover-size);
        --height: var(--cover-size);
        --shadow-gradient: linear-gradient(to left, #0006 -3%, transparent 2% 98%, #0009 104%);
    ...
    `}}customElements.define("cover-js",i);class n extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}static get styles(){return`
      :host {
        --width: var(--cover-size);
        --height: var(--cover-size);
    ...
    `}}customElements.define("cover-terminal",n);const h={html:`
...
`,postcss:`
  ...
  `};class s extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}static get styles(){return`
    ...
    `}connectedCallback(){this.logo=this.getAttribute("logo"),this.render();const t=this.shadowRoot.querySelector(".container"),c=document.createElement(`cover-${this.logo}`);c.classList.add("box"),t.insertAdjacentElement("afterbegin",c)}render(){this.shadowRoot.innerHTML=`
